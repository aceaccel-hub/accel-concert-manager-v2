...
import { Plus, Trash2, UserPlus, Star, Check, X, Pencil } from 'lucide-react';
...
export default function MembersTab({ concertId }: Props) {
  ...
  // 등급 즉시 변경
  const handleGradeChange = async (cm: ConcertMemberFull, grade: 'A' | 'B' | 'C' | undefined) => {
    const updated = { ...cm, concertGrade: grade };
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { member: _m, ...cmData } = updated;
    await db.concertMembers.put(cmData);
  ...
  // 인라인 수정 시작
  const startEdit = (cm: ConcertMemberFull) => {
    setEditingId(cm.id);
    setEditForm({
      part: cm.part || cm.member?.part || '',
      role: cm.role || cm.member?.role || '',
      fee: cm.fee,
  ...
  // 인라인 저장
  const saveEdit = async (cm: ConcertMemberFull) => {
    // concertMembers 업데이트
    const { member: _m, ...cmData } = cm;
    await db.concertMembers.put({
      ...cmData,
      part: editForm.part as string,
      role: editForm.role as string,
      fee: editForm.fee,
    });
    // members DB에도 전화번호 반영
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {concertMembers.map(cm => {
                ...
                return (
                  <tr key={cm.id} className={`${cm.isReserve ? 'opacity-60' : ''} ${isEditing ? 'bg-indigo-50/40' : 'hover:bg-gray-50'}`}>
                    {/* 이름 */}
                    ...
                    <td className="px-4 py-3">
                      <GradeSelector
                        grade={cm.concertGrade}
                        onChange={(g) => handleGradeChange(cm, g)}
                      />
                    </td>
                    ...
                    <td className="px-4 py-3 text-gray-600">
                      {isEditing ? (
                        ...
                        >
                          {['악장', '수석', '부수석', '일반단원', '객원', '지휘자', '협연자'].map(r => (
...
function GradeSelector({ grade, onChange }: {
  grade?: 'A' | 'B' | 'C';
  onChange: (g: 'A' | 'B' | 'C' | undefined) => void;
}) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className={`inline-flex items-center justify-center w-8 h-6 rounded-md text-xs font-bold transition-colors border ${
          cfg ? `${cfg.bg} border-transparent` : 'bg-gray-100 text-gray-400 border-gray-200 hover:bg-gray-200'
        }`}
        title="등급 변경"
      >
        {grade ?? '—'}
        ...
        <div className="absolute left-0 top-7 z-20 bg-white border border-gray-200 rounded-xl shadow-xl p-1.5 flex gap-1">
          {(['A', 'B', 'C'] as const).map(g => (
            <button
              ...
              className={`w-8 h-7 rounded-lg text-xs font-bold transition-colors ${gradeConfig[g].bg} ${gradeConfig[g].hover}`}
            >
              {g}
            </button>
          ))}
          {grade && (
            <button
              ...
              className="w-8 h-7 rounded-lg text-xs text-gray-400 hover:bg-gray-100"
              title="등급 해제"
            >
              ✕
            </button>
