                  ...
                  <tr key={b.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => setEditItem(b)} className="text-gray-400 hover:text-indigo-600"><Edit2 size={14} /></button>
                        <button onClick={() => handleDelete(b.id)} className="text-gray-400 hover:text-red-600"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                );
...
function BudgetForm({ concertId, item, onClose, onSaved }: {
  ...
  const [form, setForm] = useState({
    type: '지출' as Budget['type'],
    category: '',
    title: '',
    plannedAmount: 0,
    paidAmount: 0,
    paymentStatus: '예정' as Budget['paymentStatus'],
    memo: '',
  });
  ...
  useEffect(() => {
    if (item) setForm({
      ...
      plannedAmount: item.plannedAmount, paidAmount: item.paidAmount,
      paymentStatus: item.paymentStatus, memo: item.memo || '',
    });
  }, []);
    ...
    <Modal title={item ? '예산 항목 편집' : '예산 항목 추가'} onClose={onClose} size="md">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="number" className="input" value={form.plannedAmount} onChange={e => set('plannedAmount', +e.target.value)} />
        </div>
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">잔액은 자동 계산됩니다: {(form.plannedAmount - form.paidAmount).toLocaleString()}원</p>
        </div>
        <div>
